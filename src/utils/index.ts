...
export const timestampToString = (
  ...
  let diffTime: string | number =
    (new Date().getTime() - (create_at || 0)) / 1000;
  if (diffTime < 60) {
    diffTime = 'Just now';
...
export const convertDateToTimeStampFireBase = (
  date: Date,
): firestore.Timestamp => {
...
export const uploadSuperImages = (
  ...
  return images.map(async (img, index) => {
    ...
    return {
      sourceId: uid,
      hashtags: Array.from(
        new Set(img.labels.filter(x => x.type === 'hashtag').map(x => x.text)),
      ),
      mention: Array.from(
        new Set(
          img.labels.filter(x => x.type === 'people').map(x => x.text.slice(1)),
        ),
      ),
      address: Array.from(
        new Set(
