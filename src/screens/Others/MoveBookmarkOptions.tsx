...
type MoveBookmarkOptionsRouteProp = RouteProp<
...
const MoveBookmarkOptions = ({route}: MoveBookmarkOptionsProps) => {
  ...
  const {bookmarks, fromCollectionName, selectedIndexs} = route.params || {};
  ...
  const _onMoveBookmark = async (targetCollectionName: string) => {
    if (collections) {
      Promise.all(
        selectedIndexs.map(async i => {
          await dispatch(
            MoveBookmarkToCollectionRequest(
              fromCollectionName,
