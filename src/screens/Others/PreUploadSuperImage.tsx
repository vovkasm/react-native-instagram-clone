...
type PreUploadSuperImageRouteProp = RouteProp<
...
const PreUploadSuperImage = ({route}: PreUploadSuperImageProps) => {
  ...
  const fetchSuggestionList = async () => {
    ...
    const getSuggestIdTasks: Promise<void>[] = rq.docs.map(
      async (rs, index) => {
    ...
    const fetchSuggestionInfo: Promise<ProfileX>[] = preList.map(
      async suggestion => {
        const rq = await ref
          .collection('users')
          .doc(`${suggestion.username}`)
          .get();
        if (rq.exists) {
          return rq.data() || {};
        } else {
            ...
            <View style={styles.sendMethodItem}>
              <View
                ...
                <View
                  ...
                  <TouchableOpacity onPress={() => navigate('CloseFriends')}>
                    <Text
                      style={{
                        color: '#666',
                      }}>
              ...
              <TouchableOpacity
                onPress={_onShareToStory.bind(
                  null,
                  storyPermissions.CLOSE_FRIENDS,
                )}
                style={styles.btnUpload}>
                <Text
                  style={{
