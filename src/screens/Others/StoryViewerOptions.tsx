...
type StoryViewerOptionsRouteProp = RouteProp<
...
const StoryViewerOptions = ({route}: StoryViewerOptionsProps) => {
  ...
  const hideStoryFrom =
    useSelector(state => state.user.setting?.privacy?.story?.hideStoryFrom) ||
    [];
  const [userInfo, setUserInfo] = useState<ProfileX>({});
  const [showConfirmBlock, setShowConfirmBlock] = useState<boolean>(false);
  const [showConfirmRemoveFollower, setShowConfirmRemoveFollower] =
        ...
        <TouchableOpacity
          ...
          <View style={styles.confirmBox}>
            ...
            <Text
              ...
              Instagram won't tell to {username} they were removed from your
              followers.
            </Text>
            <TouchableOpacity
