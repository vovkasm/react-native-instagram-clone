...
type StorySeenListRouteProp = RouteProp<
...
const StorySeenList = ({route}: StorySeenListProps) => {
  ...
  const [targetSendMsgUsername, setTargetSendMsgUsername] =
    useState<string>('');
  ...
  useEffect(() => {
    (async () => {
      ...
      const fetchUserInfoTask: Promise<ProfileX>[] = filteredSeenList.map(
        async usr => {
  ...
  let seenCount =
    filteredSeenList.length < 1000
      ? `${filteredSeenList.length}`
        ...
        <KeyboardAvoidingView behavior="position" style={styles.msgInput}>
          <View
            ...
            <View style={styles.msgUserInfo}>
              <FastImage
                source={{
                  uri: viewerInfos.find(
                    x => x.username === targetSendMsgUsername,
                  )?.avatarURL,
                }}
                style={{
                  height: 64,
            ...
            />
            {msgTxt.length > 0 && (
              <TouchableOpacity onPress={_onSendMsg} style={styles.btnSend}>
                <Text
                  style={{
                    fontSize: 16,
                    fontWeight: '500',
...
const ViewerItem = React.memo(
  ({user, setTargetMsgUsername}: ViewerItemProps) => {
