...
import {
  ...
  UndoMyLastMessageRequest,
} from '../../actions/messageActions';
import {useSelector} from '../../reducers';
type ShareToDirectRouteProp = RouteProp<
...
const ShareToDirect = ({route}: ShareToDirectProps) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const currentMsgUsers =
        store.getState().messages.map(x => x.ownUser.username) || [];
      const commonUsers = Array.from(
        new Set(currentMsgUsers.concat(followers).concat(followings)),
      );
      const fetchUserInfoTasks: Promise<ProfileX>[] = commonUsers.map(
        async usr => {
      ...
      const filteredUserInfoList = userInfoList.filter(
        usr =>
  ...
  const _onGestureEventHandler = ({
    ...
    if (translationY < -(0.4 * SCREEN_HEIGHT - STATUS_BAR_HEIGHT)) {
      return;
    }
            ...
            <View style={styles.messageInputWrapper}>
              <FastImage
                ...
                source={{
                  uri:
                    type === 2
                      ? ((shareItem as ExtraPost).source || [])[0].uri
