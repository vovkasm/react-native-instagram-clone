...
type ConversationOptionsRouteProp = RouteProp<
...
const ConversationOptions = ({route}: ConversationOptionsProps) => {
  ...
  const myFollowings =
    useSelector(state => state.user.extraInfo?.followings) || [];
  ...
  const sharedImages = [...conversation.messageList].filter(
    x => x.type === messagesTypes.IMAGE,
  );
  const previewSharedImages = [...sharedImages].splice(-4);
  const fType =
    myFollowings.indexOf(targetUsername) > -1
      ? 1
      : conversation.ownUser.requestedList &&
        conversation.ownUser.requestedList?.indexOf(myUsername) > -1
      ? 3
      : 2;
  ...
  //EFFECTS
  useEffect(() => {
    const currentMutedMessages = [
      ...(store.getState().user.setting?.privacy?.mutedMessages
        ?.mutedMessages || []),
    ];
    if (mutedMessages) {
  ...
  useEffect(() => {
    const currentMutedVideoCalls = [
      ...(store.getState().user.setting?.privacy?.mutedVideoCalls
        ?.mutedVideoCalls || []),
    ];
    if (mutedVideoChats) {
        ...
        <TouchableOpacity
          ...
          <View style={styles.confirmBox}>
            <Text
              ...
              Block {conversation.ownUser.fullname}?
            </Text>
            <Text
              style={{
                textAlign: 'center',
              ...
              Instagram. Instagram wont't let them know you blocked them.
            </Text>
            <TouchableOpacity
