...
import {
  UnfollowRequest,
  AddPostArchiveRequest,
  RemovePostArchiveRequest,
...
const PostOptions = ({route}: PostOptionsProps) => {
  ...
  const post = setPost
    ? item
    : useSelector(state => state.postList).filter(
        post => post.uid === item.uid,
      )[0] || {};
  ...
  const _toggleNotification = async () => {
    const rq = await firestore().collection('posts').doc(`${post.uid}`).get();
    ...
    if (index > -1) {
      ...
      notifications.push(user?.username || '');
    }
    if (setPost) {
      rq.ref.update({
          ...
          <>
            ...
            <View style={{backgroundColor: '#000'}}>
              <TouchableOpacity
                ...
                <Text>
                  Turn{' '}
                  {post.notificationUsers &&
                  post.notificationUsers?.indexOf(user?.username || '') > -1
                    ? 'Off'
                    : 'On'}{' '}
                  Post Notifications
                </Text>
