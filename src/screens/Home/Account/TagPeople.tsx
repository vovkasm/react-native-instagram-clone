...
type TagPeopleNavigationProp = StackNavigationProp<
...
export default function TagPeople({navigation, route}: TagPeopleProps) {
    ...
    if (query.length > 0) {
      ...
      ref.current.timeout = setTimeout(async () => {
        ...
        rq.docs.map(user => {
          ...
          const didntTag = images[ref.current.currentItem].tags.every(
            tag => tag.username !== data.username,
          );
          if (data.username !== myInfo.username && didntTag) {
            const type = data.privacySetting?.tags?.allowTagFrom as
              | undefined
              | 0
              | 1
              | 2;
    ...
    if (!tagging) {
      ...
      const curImageWidth =
        img.height > img.width
          ? (img.width * SCREEN_WIDTH) / img.height
          : SCREEN_WIDTH;
      const curImageHeight =
        img.height > img.width
          ? SCREEN_WIDTH
          : (img.height * SCREEN_WIDTH) / img.width;
  ...
  const _toggleShowDelBtn = (index: number, preX: number, preY: number) => {
    const imgs = [...images];
  ...
  const _updateLabelSize = (index: number, width: number, height: number) => {
    const imgs = [...images];
    const img = {...images[ref.current.currentItem]};
  ...
  return (
    <SafeAreaView style={styles.container}>
      {!tagging ? (
        ...
        <View style={styles.navigationBar}>
          <TouchableOpacity onPress={_onGoBack} style={styles.btnNavigation}>
            <Icon name="arrow-left" size={20} />
          </TouchableOpacity>
          <TextInput
            value={query}
        ...
        {images.map((img, index) => (
          <View style={styles.imageWrapper} key={index}>
            <TouchableOpacity
              ...
              {ref.current.currentItem === index &&
                images[ref.current.currentItem].tags.map((tag, tagIndex) => (
                  <Animated.View
                    onLayout={({
                      nativeEvent: {
                        layout: {width, height},
                    ...
                    <TouchableOpacity
                      onPress={() => _toggleShowDelBtn(tagIndex, tag.x, tag.y)}>
                      <PanGestureHandler
                        onGestureEvent={({
                        ...
                        onHandlerStateChange={_onEndDragLabel.bind(
                          null,
                          tagIndex,
                          tag.x,
                          tag.y,
