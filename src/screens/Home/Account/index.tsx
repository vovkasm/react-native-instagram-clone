...
const index = () => {
  ...
  const photos = useSelector(state => state.user.photos || []).filter(
    x => !archivePosts.find(y => y.uid === x.uid),
  );
  ...
  useEffect(() => {
    if (photos) {
      FastImage.preload(
        photos.map(post => ({uri: post.source ? post.source[0].uri : ''})),
      );
    }
    (async () => {
      setRefreshing(true);
  ...
  const _onScrollEndDragContainerScroll = ({
    nativeEvent: {
      contentOffset: {x},
    },
      ...
      <ScrollView
        ...
        <View style={styles.profileContainer}>
          <Animated.View
            ...
            <TouchableOpacity style={styles.btnSwitchAccount}>
              <Text
                style={{
                  fontWeight: '500',
                  fontSize: 18,
          ...
          <ScrollView
            ...
            <TouchableOpacity activeOpacity={1} onPress={_onBackToMainScreen}>
                ...
                <View
                  ...
                  <Text
                    ...
                    <Text
                      ...
                      {recommendTasks.filter(x => x.done).length} OF{' '}
