...
const AddToSavedCollection = ({route}: AddToSavedCollectionProps) => {
  ...
  const bookmarks = [...(collection?.bookmarks || [])].filter(x =>
    currentBookmarks.every(y => y.postId !== x.postId),
  );
  bookmarks.reverse();
  //
  ...
  const _onDone = () => {
    if (selectedIndexs.length > 0) {
      ...
      dispatch(
        AddBookmarkToCollectionRequest(collectionName, additionBookmarks),
      );
    }
    goBack();
  };
