...
const FromInstagram = (): JSX.Element => {
  ...
  const fromInstagram = useSelector(
    state => state.user.setting?.notification?.fromInstagram,
  );
  ...
  const _onChangeProductAnnouncements = React.useCallback(
  ...
  const _onChangeSupportRequests = React.useCallback(
    (value: NotificationLevel) => {
      dispatch(
        UpdateNotificationSettingsRequest({
          fromInstagram: {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={styles.optionsWrapper}>
          ...
          />
          <Text style={{fontSize: 12, color: '#666', paddingHorizontal: 15}}>
            Download Boomerang, Instagram's latest app.
          </Text>
        </View>
        <View style={styles.optionsWrapper}>
          <View style={styles.settingItem}>
