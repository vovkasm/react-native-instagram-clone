...
const index = (): JSX.Element => {
  ...
  const notificationSetting = useSelector(
    state => state.user.setting?.notification,
  );
  const [showingTurnOffNotificationModal, setShowingTurnOffNotificationModal] =
    useState<boolean>(false);
  const [switchOn, setSwitchOn] = useState<boolean>(
    notificationSetting?.pauseAll?.active || false,
  );
  const route = useRoute();
  const currNavigation = settingNavigationMap.filter(
    x => x.navigationName === route.name,
  )[0];
  ...
  return (
    <>
        ...
        <TouchableOpacity
          ...
          <View style={styles.pausAllOptionsWrapper}>
            <Text
              ...
              You won't get push notifications, but you 'll be able to see new
              notifications when you open Instagram .
            </Text>
      ...
      <SafeAreaView
        ...
        <NavigationBar
          title={
            settingNavigationMap.filter(x => x.navigationName === route.name)[0]
              .name
          }
          callback={() => {
            navigation.goBack();
        ...
        <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
          ...
          {currNavigation.child &&
            currNavigation.child.map((settingNavigation, index) => (
              <View
                key={index}
                style={{
                  backgroundColor: '#000',
