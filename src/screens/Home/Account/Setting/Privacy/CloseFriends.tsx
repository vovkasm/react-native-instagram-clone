...
const CloseFriends = (): JSX.Element => {
  ...
  const _onRefresh = () => {
    setCloseList(
      store.getState().user.setting?.privacy?.closeFriends?.closeFriends || [],
    );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Animated.View
        ...
        <Text
          ...
          We don't send notifications when you edit yur close friends list.{' '}
          <Text
            style={{
              color: '#000',
              fontWeight: '600',
      ...
      <ScrollView
        ...
        {!searching && (
          <View style={styles.closeFriends}>
            <View
              style={{
                padding: 15,
                flexDirection: 'row',
        ...
        <View style={styles.suggestions}>
          ...
          {result.length === 0 &&
            suggestionUsers.map((userX, index) => (
              <UserItem
                userX={userX}
                key={index}
                _onToggleCloseFriend={_onToggleCloseFriend}
...
const UserItem = ({
  userX,
  closeList,
  _onToggleCloseFriend,
}: {
  _onToggleCloseFriend: (username: string) => void;
  userX: UserInfo;
  closeList: string[];
}) => {
  return (
    <View style={styles.peopleItem}>
      ...
      <TouchableOpacity
        ...
        style={{
          ...
          backgroundColor:
            closeList.indexOf(userX.username || '') > -1 ? '#fff' : '#318bfb',
        }}>
        <Text
          style={{
            fontWeight: 'bold',
            fontSize: 16,
            color:
              closeList.indexOf(userX.username || '') > -1 ? '#000' : '#fff',
          }}>
          {closeList.indexOf(userX.username || '') > -1 ? 'Remove' : 'Add'}
        </Text>
      </TouchableOpacity>
    </View>
  );
