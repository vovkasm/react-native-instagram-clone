...
const BlockedAccounts = (): JSX.Element => {
  ...
  useEffect(() => {
    const ref = firestore();
    if (blockList && blockList.length === 0) return setBlockedUsers([]);
    if (blockList !== undefined) {
      const tasks: Promise<UserInfo>[] = blockList.map(
    ...
    <View style={styles.peopleItem}>
      ...
      <TouchableOpacity
        ...
        style={{
          ...
          backgroundColor:
            blockList.indexOf(userX.username || '') < 0 ? '#318bfb' : '#fff',
        }}>
        <Text
          style={{
            fontWeight: '600',
            color:
              blockList.indexOf(userX.username || '') < 0 ? '#fff' : '#000',
          }}>
          {blockList.indexOf(userX.username || '') < 0 ? 'Block' : 'Unblock'}
        </Text>
      </TouchableOpacity>
    </View>
  );
