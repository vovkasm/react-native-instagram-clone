...
const StoryPravicy = (): JSX.Element => {
  ...
  const [saveToGallery, setSaveToGallery] = useState<boolean>(
    story?.saveToGallery || false,
  );
  const [saveToArchive, setSaveToArchive] = useState<boolean>(
    story?.saveToArchive || false,
  );
  const [allowResharing, setAllowResharing] = useState<boolean>(
    story?.allowResharing || false,
  );
  const [allowSharing, setAllowSharing] = useState<boolean>(
    story?.allowSharing || false,
  );
  const [shareYourStoryToFacebook, setShareYourStoryToFacebook] =
    useState<boolean>(story?.shareYourStoryToFacebook || false);
  const [allowMessageReplies, setAllowMessageReplies] = useState<0 | 1 | 2>(
    story?.allowMessageReplies || 0,
  );
  const [currNavigation, setCurrNavigation] = useState<
    SettingNavigation | {name: string}
  >({name: ''});
  useEffect(() => {
    settingNavigationMap.every(settingNavigation => {
      if (settingNavigation.child) {
        return (
          settingNavigation.child.every(childSettingNavigation => {
  ...
  return (
    <View
      ...
      <View style={styles.container}>
        <NavigationBar
          title={(currNavigation as {name: string}).name}
          callback={() => {
            navigation.goBack();
          }}
        />
        <ScrollView
          ...
          <View
            ...
            <TouchableOpacity
              ...
              <Text style={{fontSize: 16}}>
                {story?.hideStoryFrom?.length || 0} People
              </Text>
            </TouchableOpacity>
            <Text
              style={{
          ...
          <View
            ...
            <TouchableOpacity
              ...
              <Text style={{fontSize: 16}}>
                {useSelector(
                  state =>
                    state.user.setting?.privacy?.closeFriends?.closeFriends,
                )?.length || 0}{' '}
                People
              </Text>
            </TouchableOpacity>
          ...
          <View
            ...
            <Text
              ...
              Automatically save photos and videos in your archive so you don't
              have to save them on our phone. Only you can see them after they
          ...
          <View
            ...
            <Text
              ...
              Other people can add your feed posts and IGTV videos to their
              stories. Your username will always show with your post.
            </Text>
            <View
              style={{
                marginVertical: 10,
            ...
            <Text
              ...
              Let your followers share photos and videos from your story as
              messages. Only you followers ca see what's shared.
            </Text>
            <View
              style={{
                marginVertical: 10,
            ...
            <Text
              ...
              Automatically share photos and videos from your story to your
              Facebook story
            </Text>
          </View>
        </ScrollView>
