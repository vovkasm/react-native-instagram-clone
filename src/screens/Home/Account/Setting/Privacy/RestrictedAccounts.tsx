...
const RestrictedAccounts = (): JSX.Element => {
  ...
  const restrictedAccounts =
    store.getState().user.setting?.privacy?.restrictedAccounts
      ?.restrictedAccounts;
  const dispatch = useDispatch();
  const [query, setQuery] = useState<string>('');
  const [searching, setSearching] = useState<boolean>(false);
  const [restrictedList, setRestrictedList] = useState<string[]>(
    restrictedAccounts || ['vucms.user1'],
  );
      ...
      {searching && (
        <View
          ...
          <ScrollView
            ...
            style={{
              height:
                SCREEN_HEIGHT - STATUS_BAR_HEIGHT - getTabBarHeight() - 44,
            }}>
...
const UserItem = ({
  ...
  restrictList: string[];
  userX: UserInfo;
  _onToggleRestrict: (username: string) => void;
}) => {
  return (
    <View style={styles.peopleItem}>
      ...
      <TouchableOpacity
        ...
        style={{
          ...
          backgroundColor:
            restrictList.indexOf(userX.username || '') < 0 ? '#318bfb' : '#fff',
        }}>
        <Text
          style={{
            fontWeight: '600',
            color:
              restrictList.indexOf(userX.username || '') < 0 ? '#fff' : '#000',
          }}>
          {restrictList.indexOf(userX.username || '') < 0
            ? 'Restrict'
            : 'Unrestrict'}
        </Text>
      </TouchableOpacity>
