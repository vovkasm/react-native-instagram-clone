...
const CreateHighlight = () => {
  ...
  const _onNext = async () => {
    if (step === 1) {
      ...
      await dispatch(
        AddStoryToHighlightRequest(storyList, name, imgUris[coverIndex]),
      );
      navigate('AccountIndex');
    }
  };
  const allowNext =
    (step === 1 && selectedIndexs.length === 0) ||
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.navigationBar}>
        <View
          ...
          <TouchableOpacity style={styles.btnNavigation} onPress={_onGoBack}>
