...
type EditHighlightRouteProp = RouteProp<
...
const EditHighlight = ({route}: EditHighlightProps) => {
  ...
  const highlight = useSelector(state =>
    state.user.highlights?.find(x => x.name === highlightName),
  );
  ...
  useEffect(() => {
    const to = setTimeout(async () => {
      ...
      const fetchTasks: Promise<StoryProcessedImage>[] = superIds.map(
        async superId => {
          const rq = await ref
        ...
        <View style={styles.navigationBar}>
          <View
            ...
            <Text
              ...
              Edit {showCovers ? 'Cover' : 'Highlight'}
            </Text>
          </View>
          {!showCovers && (
            <TouchableOpacity
                ...
                <TouchableOpacity
                  ...
                  <View
                    style={{
                      ...
                      zIndex: 1,
                      backgroundColor:
                        selectedIndexs.indexOf(
                          stories.findIndex(x => x.uid === item.uid),
                        ) > -1
                          ? '#318bfb'
                          : 'rgba(0,0,0,0.3)',
                    }}>
