...
type NotificationOptionsRouteProp = RouteProp<
...
const NotificationOptions = ({route}: NotificationOptionsProps) => {
  ...
  const notifyPostAccounts =
    setting?.notification?.notificationAccounts?.posts || [];
  const notifyStoryAccounts =
    setting?.notification?.notificationAccounts?.story || [];
  const [notifyPost, setNotifyPost] = useState<boolean>(
    notifyPostAccounts.indexOf(targetUser.username || '') > -1,
  );
  const [notifyStories, setNotifyStories] = useState<boolean>(
    notifyStoryAccounts.indexOf(targetUser.username || '') > -1,
  );
