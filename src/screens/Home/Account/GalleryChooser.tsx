  ...
  useEffect(() => {
    if (selectedGroupIndex > -1) {
      CameraRoll.getPhotos({
        ...
        const photos = result.edges;
        setPhotos(photos);
        if (photos.length > 0 && selectedIndex < 0) {
          setSelectedIndex(0);
        }
      });
    }
  ...
  const _onDone = async () => {
    ...
    if (isChooseProfilePhoto) {
      const cropData: ImageCropData = {
        ...
        size: {
          width: SCREEN_WIDTH / ref.current.currentPhoto.preRatio,
          height: SCREEN_WIDTH / ref.current.currentPhoto.preRatio,
        },
      };
      ...
      if (step === 1) {
        ...
        const tasks: Promise<ProcessedImage>[] = photoList.map(
          async (index, rIndex) => {
        ...
        const tasks: Promise<PostImage>[] = imageList.map(
          async (img, index) => {
            const blob = await uriToBlob(img.uri);
            const rq = await storage()
              .ref(
                `posts/${user?.username || 'others'}/${
      ...
      <View
        ...
        <TouchableOpacity
          ...
          <Text
            ...
            {isChooseProfilePhoto ? 'Done' : step === 1 ? 'Next' : 'Share'}
          </Text>
        </TouchableOpacity>
      </View>
      {step === 1 && (
      ...
      {step === 2 && (
        <ScrollView
          ...
          <View
            ...
            <TouchableOpacity
              ...
              <Text
                ...
                style={{
                  fontSize: 16,
                  color:
                    (address.place_name || '').length > 0 ? '#318bfb' : '#000',
            ...
            <TouchableOpacity
              onPress={setShowAdvancedSettings.bind(
                null,
            ...
            <>
              <View style={styles.settingWrapper}>
                ...
                <Text
                  ...
                  You can change this later by going to option menu at the top
                  of your post.
                </Text>
              </View>
              <View style={styles.settingWrapper}>
