...
import {
  FetchProfileXRequest,
  ResetProfileXRequest,
} from '../../../actions/profileXActions';
...
const ProfileX = ({route}: ProfileXProps) => {
  ...
  useEffect(() => {
    const type =
      userX.requestedList &&
      userX.requestedList.indexOf(me?.username || '') > -1
        ? 3
        : me?.followings && me.followings.indexOf(userXname) > -1
        ? 1
        : 2;
    setFollowType(type);
  }, [userX]);
  ...
  const _onScrollEndDragGalleryTabScroll = ({
    nativeEvent: {
      contentOffset: {x},
    },
  ...
  const _showPopupImage = (
    e: {pX: number; pY: number; w: number; h: number},
    photo: Post,
  ) => {
  ...
  const _onVerticalScrollViewScroll = ({
    nativeEvent: {
      contentOffset: {y},
    },
  }: NativeSyntheticEvent<NativeScrollEvent>) => {
      ...
      <SafeAreaView
        ...
        <View
          style={{
            ...
            width: '100%',
            height:
              SCREEN_HEIGHT - getStatusBarHeight() - 44 - STATUS_BAR_HEIGHT,
          }}>
          <Image
            style={{
              height: 80,
  ...
  const _onViewFollow = () => {};
  const posts = (userX.posts || []).filter(
    x => !(userX.archive?.posts || []).find(y => y.uid === x.uid),
  );
  return (
    <SafeAreaView style={styles.container}>
            ...
            <Animated.View
              ...
              <Animated.View
                ...
                <FastImage
                  ...
                  source={{
                    uri: selectedPhoto.source[0].uri,
      ...
      <View style={styles.profileContainer}>
        <Animated.View
          ...
          <View style={styles.btnSwitchAccount}>
          ...
          <TouchableOpacity onPress={_onShowOptions} style={styles.btnOptions}>
        ...
        <ScrollView
          ...
          <TouchableOpacity activeOpacity={1}>
            <View onLayout={_onSetHeaderHeight}>
              ...
              <View style={styles.actionsWrapper}>
                <TouchableOpacity
                  ...
                  style={{
                    ...styles.btnAction,
                    backgroundColor:
                      followType === 1 || followType === 3 ? '#fff' : '#318bfb',
                    borderWidth: followType === 1 || followType === 3 ? 1 : 0,
                  }}>
                  <Text
                    style={{
                      color:
                        followType === 1 || followType === 3 ? '#000' : '#fff',
                      fontWeight: '600',
                    }}>
                    {followType === 1
                      ? 'Following'
              ...
              {highlights.length > 0 && (
                <HighlightPreviewList highlights={highlights} />
              )}
            </View>
