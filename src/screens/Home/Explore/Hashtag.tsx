...
const Hashtag = ({route}: HashtagProps) => {
  ...
  const _onToggleFollow = async () => {
    ...
    if (index > -1) {
      ...
      currentFollowers.push(myUsername);
    }
    rq.ref.update({
...
export const PhotoItem = React.memo(({item, index}: PhotoItemProps) => {
  return (
    <TouchableOpacity
      onPress={() =>
        navigate('PostDetail', {
...
async function fetchHashtagInfo(
  hashtag: string,
  setHashtagInfo: React.Dispatch<React.SetStateAction<HashTag>>,
  setHashtagPosts: React.Dispatch<React.SetStateAction<Post[]>>,
) {
  const ref = firestore();
  const rq = await ref.collection('hashtags').doc(`${hashtag}`).get();
  if (rq.exists) {
