...
const ProfileXFollowing = ({userX}: ProfileXMutualProps) => {
  ...
  const [filteredFollowingUsers, setFiltredFollowingUsers] = useState<
    MixedProfileX[]
  >([]);
    ...
    if (query.length > 0) {
      ref.current.queryTimeout = setTimeout(() => {
        ...
        const temp = followers.filter(
          usr => (usr.username || '').indexOf(query) > -1,
        );
        setFiltredFollowingUsers(temp);
      }, 300);
    } else {
      setFiltredFollowingUsers([...followingUsers]);
    }
  }, [query]);
  useEffect(() => {
    if (userX.followings) {
      ...
      const ref = firestore();
      const fetchFollowingUserTasks: Promise<MixedProfileX>[] = followings.map(
        async usr => {
