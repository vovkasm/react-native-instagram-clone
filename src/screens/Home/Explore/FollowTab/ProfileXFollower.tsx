...
const ProfileXFollower = ({userX}: ProfileXMutualProps) => {
  ...
  const [filteredFollowerUsers, setFiltredFollowerUsers] = useState<
    MixedProfileX[]
  >([]);
  const ref = useRef<{
  ...
  useEffect(() => {
    if (userX.followers) {
      ...
      const ref = firestore();
      const fetchFollowerUserTasks: Promise<MixedProfileX>[] = followers.map(
        async usr => {
  ...
  useEffect(() => {
    ...
    if (query.length > 0) {
      ...
      setFiltredFollowerUsers([...followerUsers]);
    }
  }, [query]);
