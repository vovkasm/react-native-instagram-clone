...
const Location = ({route}: LocationProps) => {
  ...
  if (addressInfo.sources) {
    postCount =
      addressInfo.sources.length < 1000
        ? `${addressInfo.sources.length}`
  ...
  return (
    <>
      ...
      <SafeAreaView style={styles.container}>
        <View>
          <NavigationBar title={address.place_name || ''} callback={goBack} />
          <TouchableOpacity
            onPress={() =>
              navigate('ShareToDirect', {
                item: {...addressInfo},
              })
            }
        ...
        <View
          ...
          <View style={styles.locationPreviewWrapper}>
            <MapView
              scrollEnabled={false}
              region={{
