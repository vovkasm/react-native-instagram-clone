...
const ForgotPassword = ({navigation}: ForgotPasswordProps) => {
  ...
  const _checkExistUsername = async () => {
    ...
    const emailCheck = await usersRef.where('email', '==', username).get();
    const usernameCheck = await usersRef
      .where('username', '==', username)
      .get();
    const phoneCheck = await usersRef.where('phone', '==', username).get();
    if (emailCheck.size + usernameCheck.size + phoneCheck.size === 0) {
      ...
      if (emailCheck.size > 0) {
        await auth().sendPasswordResetEmail(emailCheck.docs[0].data().email);
      } else if (usernameCheck.size > 0) {
        await auth().sendPasswordResetEmail(usernameCheck.docs[0].data().email);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View>
        <NavigationBar
          title={sentReset ? 'Access Account' : 'Login Help'}
        ...
        <View style={styles.afterSentContainer}>
          <View style={styles.infoLine}>
            ...
            <Text
              ...
              {sendingReset ? 'Sending Email' : 'Sent Email'}
            </Text>
            {sendingReset && (
              <Animated.Image
          ...
          <TouchableOpacity
            activeOpacity={0.8}
            style={styles.sendingDescription}>
            <Text
              style={{
                fontWeight: '300',
              }}>
              An email has been sent to your account's email address. Please
              check your email to continue. If you are still having problems ,
              please visit the{' '}
              <Text
                style={{
                  fontWeight: '500',
