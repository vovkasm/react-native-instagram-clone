...
export const TriggerMessageListenerRequest = (): ThunkAction<
  Promise<void>,
  {},
  {},
  MessageAction
      ...
      dbRef.ref(`/messages/${myUsernamePath}/`).once('value', async snap => {
        ...
        snap.forEach(targetUser => {
          if (targetUser.key !== 'forceUpdate') {
            ...
            if (userIds.indexOf(`${targetUser.key}`) < 0) userIds.push(userId);
        ...
        Promise.all(fetchMyMessagesTasks).then(async () => {
          const preUserInfos: ProfileX[] = store
            .getState()
            .messages.map(x => x.ownUser);
          const fetchUserInfoListTasks: Promise<ProfileX>[] = userIds.map(
            async userId => {
              let idx = -1;
          ...
          const userInfos: ProfileX[] = await Promise.all(
            fetchUserInfoListTasks,
          );
          const onlineStatus: OnlineStatus[] = await Promise.all(
            fetchUserStatusTasks,
          );
          const collection: MessageList = messageCollection.map(
            (messageGroup, index) => {
              return {
...
export const TriggerMessageListenerSuccess = (
  payload: MessageList,
): MessageSuccessAction<MessageList> => {
  return {
...
export const CreateMessageRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
      const targetUsernamePath =
        convertToFirebaseDatabasePathName(targetUsername);
      const myUsername = store.getState().user.user.userInfo?.username || '';
      const myUsernamePath = convertToFirebaseDatabasePathName(myUsername);
      const dbRef = database();
      const ref = firestore();
...
export const CreateEmptyConversationRequest = (
  targetUsername: string,
): ThunkAction<Promise<void>, {}, {}, MessageAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
...
export const AddEmoijToMessageRequest = (
  targetUsername: string,
  msgId: number,
  emoji: number,
): ThunkAction<Promise<void>, {}, {}, MessageAction> => {
...
export const RemoveEmoijToMessageRequest = (
  targetUsername: string,
  msgId: number,
): ThunkAction<Promise<void>, {}, {}, MessageAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
...
export const UndoMyLastMessageRequest = (
  targetUsername: string,
): ThunkAction<Promise<void>, {}, {}, MessageAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
