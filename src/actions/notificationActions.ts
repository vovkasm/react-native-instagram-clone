...
export const FetchNotificationListRequest = (): ThunkAction<
  Promise<void>,
  {},
  {},
  NotificationAction
  ...
  return async (dispatch: ThunkDispatch<{}, {}, NotificationAction>) => {
    try {
      ...
      const rq = await ref
        ...
        .get();
      const notificationTasks: Promise<ExtraNotification>[] = rq.docs.map(
        async doc => {
...
export const FetchNotificationListSuccess = (
  payload: NotificationList,
): NotificationSuccessAction<NotificationList> => {
  return {
    ...
    try {
      ...
      if (
        ...
        if (rq.size > 0) {
          ...
          const currentFroms =
            (targetNotification.data().froms as string[]) || [];
