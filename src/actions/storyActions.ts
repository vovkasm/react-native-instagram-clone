...
export const FetchStoryListRequest = (): ThunkAction<
  Promise<void>,
  {},
  {},
  StoryAction
    ...
    try {
      ...
      if (request.exists) {
        ...
        const fullStory: StoryList = ownInfos.map(info => {
          const collectStory: Story[] = collection.filter(
...
export const FetchStoryListSuccess = (
  payload: StoryList,
): StorySuccessAction => {
  return {
...
export const PostStoryRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, StoryAction>) => {
    try {
      ...
      const me = store.getState().user.user;
      const shouldSaving =
        !!store.getState().user.setting?.privacy?.story?.saveToArchive;
