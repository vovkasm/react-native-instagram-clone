...
export function useSuggestion(
  ...
  const fetchSuggestion = async () => {
    ...
    rs.docs.map(post => {
      ...
      data.likes?.map(usr => recentInteractions.push(usr));
      data.commentList?.map(usr => recentInteractions.push(usr));
    });
    recentInteractions = Array.from(new Set(recentInteractions)).filter(
      usr =>
        usr !== myUsername &&
    ...
    if (limit) {
      recentInteractionsForTask = recentInteractionsForTask.splice(
        0,
        limit - recentInteractions.length,
      );
    }
    ...
    const recentInteractionTasks: Promise<ExtraSuggestionUserInfo>[] =
      recentInteractionsForTask.map(usr => asyncFetchUser(usr, 1));

    let dontFollowUsrnames = [...followerUsrnames].filter(
      usr =>
        follwingUsrnames.indexOf(usr) < 0 &&
    ...
    if (limit) {
      dontFolloweUsernameForTask = dontFolloweUsernameForTask.splice(
        0,
        limit - recentInteractionsForTask.length,
      );
    }
    const dontFollowBackTasks: Promise<ExtraSuggestionUserInfo>[] =
      dontFolloweUsernameForTask.map(usr => asyncFetchUser(usr, 2));
    Promise.all(recentInteractionTasks.concat(dontFollowBackTasks)).then(
      result => {
        let followedByInteractionList: string[] = [];
        result.map(usr => {
          if (usr.type === 1) {
            followedByInteractionList = followedByInteractionList.concat(
              (usr.followings || []).splice(0, 5),
            );
          }
        });
        followedByInteractionList = Array.from(
          new Set(followedByInteractionList),
        ).filter(
          usr =>
            usr !== myUsername &&
            follwingUsrnames.indexOf(usr) < 0 &&
        ...
        if (limit) {
          followedByInteractionForTask = followedByInteractionForTask.splice(
            0,
            limit -
              recentInteractionsForTask.length -
              dontFolloweUsernameForTask.length,
          );
        }
        const followedByInteractionTasks: Promise<ExtraSuggestionUserInfo>[] =
          followedByInteractionForTask.map(usr => asyncFetchUser(usr, 4));
        ...
        Promise.all(followedByInteractionTasks).then(result2 => {
          const followerOfFollowingUsernameTasks: Promise<string>[] =
            follwingUsrnames.map(async userX => {
              ...
              if (userFollowings.length > 0) {
                let randomPickusername =
                  userFollowings[
                    Math.floor(Math.random() * userFollowings.length)
                  ];
          ...
          Promise.all(followerOfFollowingUsernameTasks).then(resultTemp => {
            let followedByFollwers = Array.from(new Set(resultTemp)).filter(
              usr =>
                usr !== myUsername &&
                usr !== '' &&
                follwingUsrnames.indexOf(usr) < 0 &&
            ...
            if (limit) {
              followedByFollwers = followedByFollwers.splice(
                0,
                limit -
                  recentInteractionsForTask.length -
                  dontFolloweUsernameForTask.length -
            ...
            const followerOfFollowingsTasks: Promise<ExtraSuggestionUserInfo>[] =
              followedByFollwers.map(usr => asyncFetchUser(usr, 3));
            Promise.all(followerOfFollowingsTasks).then(result3 => {
              ...
              result = result
                .map(usr => {
                  if (
                    usr.requestedList &&
                    usr.requestedList.indexOf(myUsername) > -1
                  ) {
                    usr.followType = 3;
                    return usr;
                  }
