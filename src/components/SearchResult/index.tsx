...
const SearchResult = ({query}: SearchResultProps) => {
  const [resultData, setResultData] = useState<
    (MixedProfileX | HashTag | MapBoxAddress)[]
  >([]);
  ...
  useEffect(() => {
    if (history) {
      history.reverse();
      const ref = firestore();
      const fetchRecentTasks: Promise<
        MixedProfileX | HashTag | MapBoxAddress
      ...
      Promise.all(fetchRecentTasks).then(result => {
        result = result.map(item => {
          if ('username' in item) {
            if (
              (item as MixedProfileX).requestedList &&
              ((item as MixedProfileX).requestedList || []).indexOf(
                myUsername,
              ) > -1
            ) {
            ...
            if (
              user?.followings &&
              user.followings.indexOf(item.username || '') > -1
            ) {
              item.followType = 1;
            } else {
              item.followType = 2;
  ...
  useEffect(() => {
    ...
    if (query.length > 0) {
      ref.current.timeout = setTimeout(async () => {
        const ref = firestore();
        const hashtag = await ref
          .collection('hashtags')
          .where('keyword', 'array-contains', query.trim())
          .get();
        const hashTagList: HashTag[] = hashtag.docs.map(
          doc => doc.data() || {},
        );
        const accounts = await ref
          ...
          .get();
        const accountList: MixedProfileX[] = accounts.docs.map(
          x => x.data() || {},
        );
        const posts = await ref
          .collection('posts')
          .where('address.keyword', 'array-contains', query)
          .get();
        ...
        temp.map((x, index) => {
          let check = addressList.every((address, index2) => {
            if (address.id === x.id && index !== index2) {
              return false;
            }
            return true;
          });
...
const SearchTab = ({resultData, recentList, searching}: SearchTabProps) => {
  const filter = (
    list: (MixedProfileX | HashTag | MapBoxAddress)[],
    field: string[],
  ) => [...list.filter(x => field.every(f => `${f}` in x))];
  ...
  return (
    <Tab.Navigator>
      <Tab.Screen
        children={() => <TopResult {...{resultData, recentList, searching}} />}
        name="Top"
      />
      <Tab.Screen
        children={() => (
