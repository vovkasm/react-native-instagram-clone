        ...
        if (animIndex === childIndex) {
          ...
          Animated.timing(animX, {
            ...
            db.collection('stories')
              .doc(`${item.storyList[animIndex].uid}`)
              .get()
              .then(storyDoc => {
                const currentSeenList: string[] =
                  (storyDoc.data() || {}).seenList || [];
          ...
          <View style={styles.topInfo}>
            ...
            {controller.currentGroupIndex === index && (
              <Text
                ...
                {timestampToString(
                  item.storyList[childIndex]?.create_at?.toMillis() || 0,
                )}
              </Text>
            )}
          </View>
...
const SeenPreviewList = ({
  ...
  useEffect(() => {
    ...
    const db = firestore();
    const fetchPreviewSeenListInfoTask: Promise<ProfileX>[] =
      previewInfoUsernames.map(async usr => {
      ...
      <View style={styles.previewSeenList}>
        {previewSeenList.map((usr, idx) => (
          <FastImage
            ...
            source={{
              uri: seenListPreviewInfo.find(x => x.username === usr)?.avatarURL,
            }}
          />
        ))}
      </View>
