...
const PhotoShower = ({sources, onChangePage}: PhotoShowerProps) => {
  ...
  const [_animTags, _] = useState<Animated.Value[][]>(
    sources.map(source => {
      return source.tags.map(tag => new Animated.Value(0));
    }),
  );
  ...
  useEffect(() => {
    if (showTags) {
      Animated.parallel(
        _animTags[currentPage - 1].map(anim =>
                      ...
                      <Animated.View
                        ...
                        style={{
                          ...
                          top: Animated.add(
                            SCREEN_RATIO * tag.y,
                            Animated.subtract(
                              (SCREEN_RATIO * tag.height) / 2,
                              Animated.divide(
                                getLabelHeight(_animTags, index, index2, tag),
                                2,
                              ),
                            ),
                          ),
                          left: Animated.add(
                            SCREEN_RATIO * tag.x,
                            Animated.subtract(
                              (SCREEN_RATIO * tag.width) / 2,
                              Animated.divide(
                                getLabelWidth(_animTags, index, index2, tag),
                                2,
                              ),
                            ),
                          ),
...
function getLabelHeight(
  ...
  index: number,
  index2: number,
  tag: {x: number; y: number; width: number; height: number; username: string},
) {
  return _animTags[index][index2].interpolate({
    inputRange: [0, 1],
