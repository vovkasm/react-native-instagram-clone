...
export async function getRecommendLabels(diffMonth: number): Promise<{
  ...
  const myUsername = `${store.getState().user.user.userInfo?.username}`;
  const currentBlockedList =
    store.getState().user.setting?.privacy?.blockedAccounts?.blockedAccounts ||
    [];
  const userRef = firestore().collection('users');
  const blockMe = await userRef
    .where(
  ...
  const posts = await postRef
    .where('create_at', '>=', new Date(from))
    .where('likes', 'array-contains', myUsername)
    .orderBy('create_at', 'desc')
    .get();
  const reactedPosts = posts.docs
    .map(x => x.data() as Post)
    .filter(
      x =>
        currentBlockedList.indexOf(`${x.userId}`) < 0 &&
        blockedMeList.indexOf(`${x.userId}`) < 0 &&
